...
function AdminMain() {
    ...
    useEffect(() => {
        if (settingInfo !== null) {
            ...
            for (let index = 0; index < 4; index++) {
                if (
                    settingInfo.find((element) => element.key === "section1_img" + (index + 1)).value === "" ||
                    settingInfo.find((element) => element.key === "section1_img" + (index + 1)).value === null
                )
                    value[index] = true;
            }
    ...
    const readMainPageContent = async () => {
        const response = await axios.get(process.env.REACT_APP_RESTAPI_HOST + "setting/admin", {
            params: {
    ...
    const readAllLecture = async () => {
        const response = await axios.get(process.env.REACT_APP_RESTAPI_HOST + "lecture/all", {
            params: {
                token: window.sessionStorage.getItem("token"),
                manageID: window.sessionStorage.getItem("id"),
            },
    ...
    const clickEditMainPageButton = async () => {
        ...
        await axios.post(process.env.REACT_APP_RESTAPI_HOST + "setting/mainHome", params, { headers }).then(function (res) {
    ...
    const clickEditExplainSiteButton = async () => {
        ...
        if (removeBackgroundSection2 === true) {
            ...
            return;
        } else if (backgroundSection2 == null) {
            //사진은 원래 꺼 두고, 문구만 수정하고 싶을때,
            // console.log("here!!");
            params.append("value", settingInfo.find((element) => element.key === "section2_img").value);
        } else {
            // console.log("11111");
            params.append("value", settingInfo.find((element) => element.key === "section2_img").value);
            // console.log("backgroundSection2 is",backgroundSection2);
        ...
        params.append("token", window.sessionStorage.getItem("token"));
        params.append("manageID", window.sessionStorage.getItem("id"));
        // console.log(params.get('value')); //2022/3/1646739839625
        // console.log(params.get('file')); //null
        // console.log(params.get('content')); //한동대학교 비전특강 홈페이지 입니다.!!!!~!~!~!
        // console.log(params.get('token')); //토큰내용들이 나옴
        // console.log(params.get('manageID')); //106

        await axios.post(process.env.REACT_APP_RESTAPI_HOST + "setting/explainSite", params, { headers }).then(function (res) {
            // console.log("!!!!!!!!!!!!");
    ...
    const clickEditExplainInstructorButton = async () => {
        ...
        await axios.post(process.env.REACT_APP_RESTAPI_HOST + "setting/explainInstructor", params, { headers }).then(function (res) {
    ...
    const clickEditExplainLectureButton = async () => {
        ...
        await axios.post(process.env.REACT_APP_RESTAPI_HOST + "setting/explainLecture", params, { headers }).then(function (res) {
            alert("강사 화면 수정이 완료되었습니다.");
            readMainPageContent();
    ...
    return (
        <div>
            ...
            <div className="admin-content-wrapper">
                <p className="main-title mb40">처음 화면</p>
                <div className="main-content-wrapper mb55">
                    ...
                    <div className="main-sub-wrapper">
                        {[0, 1, 2, 3].map((data, i) => (
                            <div className="img-wrapper" key={i}>
                                <span className="default-img">
                                    {removeBackgroundImage[data] === false && backgroundImage[data] !== null ? (
                                        <img
                                            ...
                                            src={backgroundBase64Image[data]}
                                            alt="배경화면"
                                            onError={(e) => {
                                      ...
                                      settingInfo !== null &&
                                      settingInfo.find((element) => element.key === "section1_img" + (data + 1)).value !== null &&
                                      settingInfo.find((element) => element.key === "section1_img" + (data + 1)).value !== "" &&
                                      settingInfo.find((element) => element.key === "section1_img" + (data + 1)).value !== "null" ? (
                                        <img
                                            className="default-img"
                                            src={process.env.REACT_APP_RESTAPI_HOST + "resources/upload/" + settingInfo.find((element) => element.key === "section1_img" + (data + 1)).value}
                                            alt="배경화면"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                                ...
                                <div>
                                    <label className={removeBackgroundImage[data] === false ? "input-file-button" : "input-file-button disabled"} htmlFor={"input-file" + data}>
                                        첨부파일
                                    </label>
                                    {removeBackgroundImage[data] === true ? (
                                        ...
                                        <input
                                            ...
                                            style={{ display: "none" }}
                                            onChange={(e) => {
                                                ChangeFiles(e, backgroundImage, setBackgroundImage, backgroundBase64Image, setBackgroundBase64Image, data);
                                            }}
                                        />
                                    )}
                    ...
                    <div className="main-sub-wrapper justify-start mb0">
                        <div className="mr30 img-wrapper no-margin">
                            <span className="default-img">
                                {removeBackgroundImage[4] === false && backgroundImage[4] !== null ? (
                                    <img
                                        className="default-img"
                                        src={backgroundBase64Image[4]}
                                        alt="배경화면"
                                        onError={(e) => {
                                            e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                        }}
                                    />
                                ) : removeBackgroundImage[4] === false &&
                                  settingInfo !== null &&
                                  settingInfo.find((element) => element.key === "section1_title").value !== null &&
                                  settingInfo.find((element) => element.key === "section1_title").value !== "null" &&
                                  settingInfo.find((element) => element.key === "section1_title").value !== "" ? (
                                    <img
                                        className="default-img"
                                        src={process.env.REACT_APP_RESTAPI_HOST + "resources/upload/" + settingInfo.find((element) => element.key === "section1_title").value}
                                        alt="배경화면 1"
                                        onError={(e) => {
                                            e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                        }}
                            ...
                            <label className={removeBackgroundImage[4] === false ? "input-file-button" : "input-file-button disabled"} htmlFor={"input-file" + 4}>
                                첨부파일
                            </label>
                            {removeBackgroundImage[4] === true ? (
                                ...
                                <input
                                    ...
                                    style={{ display: "none" }}
                                    onChange={(e) => {
                                        ChangeFiles(e, backgroundImage, setBackgroundImage, backgroundBase64Image, setBackgroundBase64Image, 4);
                                    }}
                                />
                            )}
                            <span className="admin-main-remove">삭제</span>
                            <input
                                type="checkbox"
                                defaultValue={removeBackgroundImage[4]}
                                onChange={(e) => {
                        ...
                        <div className="position-r"></div>
                    </div>
                </div>
                <div className="button-wrapper">
                    <GreyButton class="inst-grey-btn" name="수정하기" click={clickEditMainPageButton} />
                </div>
                <p className="main-title mb30">사이트 설명 화면</p>
                <div className="main-content-wrapper mb55">
                    ...
                    <div className="main-sub-wrapper">
                        <div className="mr25 img-wrapper">
                            <span className="default-img">
                                {
                                    removeBackgroundSection2 === false && backgroundSection2 !== null ? (
                                        <img
                                            className="default-img"
                                            src={backgroundBase64Section2}
                                            alt="배경화면 section2"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                                        />
                                    ) : removeBackgroundSection2 === false &&
                                      settingInfo !== null &&
                                      settingInfo.find((element) => element.key === "section2_img").value !== null &&
                                      settingInfo.find((element) => element.key === "section2_img").value !== "null" &&
                                      settingInfo.find((element) => element.key === "section2_img").value !== "" ? (
                                        <img
                                            className="default-img"
                                            src={process.env.REACT_APP_RESTAPI_HOST + "resources/upload/" + settingInfo.find((element) => element.key === "section2_img").value}
                                            alt="배경화면 section 2"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                            ...
                            <label className={removeBackgroundSection2 === false ? "input-file-button" : "input-file-button disabled"} htmlFor="input-file-explain">
                                첨부파일
                            </label>
                            <input
                                type="file"
                                id="input-file-explain"
                                style={{ display: "none" }}
                                onChange={(e) => {
                                    handleChangeFile(e, setBackgroundSection2, setBackgroundBase64Section2);
                                }}
                            />
                            ...
                            <input
                                type="checkbox"
                                onChange={(e) => {
                                    setRemoveBackgroundSection2(e.target.checked);
                                }}
                            />
                        </div>
                    </div>
                    ...
                    <div>
                        <textarea
                            ...
                            placeholder="행사 조직 안내글을 입력하세요."
                            value={section2Content || ""}
                            onChange={(e) => {
                                setSection2Content(e.target.value);
                            }}
                        ></textarea>
                    </div>
                </div>
                <div className="button-wrapper">
                    <GreyButton class="inst-grey-btn" name="수정하기" click={clickEditExplainSiteButton} />
                </div>

                <p className="main-title mb30">대표 강사 소개 화면</p>
                            ...
                            <div className="img-inst-wrapper" key={i}>
                                <span className="default-inst-img">
                                    {instructorId[data] !== -1 && instructorBase64Image[data] !== null && instructorBase64Image[data] !== "" ? (
                                        <img
                                            className="default-img admin-main-img-content"
                                            src={instructorBase64Image[data]}
                                            alt="강사화면1"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                                        />
                                    ) : instructorFlag[data] === false &&
                                      defaultInstructorImage[data] === true &&
                                      settingInfo !== null &&
                                      settingInfo.find((element) => element.key === "section3_img" + (data + 1)).value !== null &&
                                      settingInfo.find((element) => element.key === "section3_img" + (data + 1)).value !== "" &&
                                      settingInfo.find((element) => element.key === "section3_img" + (data + 1)).value !== "null" ? (
                                        <img
                                            className="default-img admin-main-img-content"
                                            src={process.env.REACT_APP_RESTAPI_HOST + "resources/upload/" + settingInfo.find((element) => element.key === "section3_img" + (data + 1)).value}
                                            alt="강사화면2"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                                      ...
                                      instructorInfo !== null &&
                                      instructorInfo.find((element) => parseInt(element.id) === instructorId[data]) !== null &&
                                      instructorInfo.find((element) => parseInt(element.id) === instructorId[data]) !== undefined &&
                                      instructorInfo.find((element) => parseInt(element.id) === instructorId[data]).image !== undefined &&
                                      instructorInfo.find((element) => parseInt(element.id) === instructorId[data]).image !== null &&
                                      instructorInfo.find((element) => parseInt(element.id) === instructorId[data]).image !== "null" &&
                                      instructorInfo.find((element) => parseInt(element.id) === instructorId[data]).image !== "" ? (
                                        <img
                                            className="default-img admin-main-img-content"
                                            src={process.env.REACT_APP_RESTAPI_HOST + "resources/upload/" + instructorInfo.find((element) => parseInt(element.id) === instructorId[data]).image}
                                            alt="강사화면3"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                                ...
                                <label className={instructorId[data] !== -1 ? "input-file-button" : "input-file-button disabled"} htmlFor={"input-inst-file" + data}>
                                    첨부파일
                                </label>
                                {instructorId[data] === -1 ? (
                                    ...
                                    <input
                                        type="file"
                                        id={"input-inst-file" + data}
                                        style={{ display: "none" }}
                                        onChange={(e) => {
                                            ChangeFiles2(e, instructorImage, setInstructorImage, instructorBase64Image, setInstructorBase64Image, data, setInstructorFlag, instructorFlag);
                                        }}
                                    />
                                )}
                                        ...
                                        >
                                            ...
                                            {instructorInfo !== null ? (
                                                instructorInfo.map((data2, index2) => (
                                                    <option key={index2} value={parseInt(data2.id)}>
                                                        {data2.inst_name}
                                                    </option>
                                                ))
                                            ) : (
                ...
                <div className="button-wrapper">
                    <GreyButton class="inst-grey-btn" name="수정하기" click={clickEditExplainInstructorButton} />
                </div>
                <p className="main-title mb30">대표 강좌 소개 화면</p>
                <div className="main-content-wrapper mb55 p4020">
                            ...
                            <div className="img-lecture-wrapper" key={i}>
                                <span className="default-lecture-img">
                                    {lectureId[data] !== -1 && lectureBase64Image[data] !== "" && lectureBase64Image[data] !== null ? (
                                      ...
                                      settingInfo !== null &&
                                      settingInfo.find((element) => element.key === "section5_img" + (data + 1)).value !== null &&
                                      settingInfo.find((element) => element.key === "section5_img" + (data + 1)).value !== "" &&
                                      settingInfo.find((element) => element.key === "section5_img" + (data + 1)).value !== "null" ? (
                                        <img
                                            className="default-img admin-main-img-content"
                                            src={process.env.REACT_APP_RESTAPI_HOST + "resources/upload/" + settingInfo.find((element) => element.key === "section5_img" + (data + 1)).value}
                                            alt="강사화면2"
                                            onError={(e) => {
                                                e.target.src = process.env.REACT_APP_DEFAULT_URL + "image/errorImage2.png";
                                            }}
                                ...
                                <label className={lectureId[data] !== -1 ? "input-file-button" : "input-file-button disabled"} htmlFor={"input-lecture-file" + data}>
                                    첨부파일
                                </label>
                                {lectureId[data] === -1 ? (
                                    ...
                                    <input
                                        type="file"
                                        id={"input-lecture-file" + data}
                                        style={{ display: "none" }}
                                        onChange={(e) => {
                                            ChangeFiles2(e, lectureImage, setLectureImage, lectureBase64Image, setLectureBase64Image, data, setLectureFlag, lectureFlag);
                                        }}
                                    />
                                )}
                ...
                <div className="button-wrapper">
                    <GreyButton class="inst-grey-btn" name="수정하기" click={clickEditExplainLectureButton} />
                </div>
            </div>
        </div>
    );
