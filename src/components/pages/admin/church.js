...
function AdminChurch() {
    ...
    const [editChurchInfo, seteditChurchInfo] = useState({
    ...
    const [addChurchInfo, setaddChurchInfo] = useState({
        userId: window.sessionStorage.getItem("id"),
        name: "",
        region_id: "",
        nation: "대한민국",
    ...
    const excelFormat = {
        ...
        district: "포항시 북구 (필수)",
        zip_code: "우편번호 (필수)",
        addr1: "경북 포항시 북구 흥해읍 한동로 558 (필수)",
        addr2: "뉴턴홀 (필수)",
        phone: "교회 번호 (필수)",
        email: "교회 이메일",
        page_url: "교회 홈페이지 주소",
        ...
        denomination: "교단",
        pastor: "담임목사",
        admin_name: "교회 담당자 이름",
        hgu_yn: "한동대와 연관성 여부",
        hgu_memo: "한동대와 연관성 세부정보",
        memo: "비고",
    };
    ...
    const createChurch = async () => {
        ...
        if (window.confirm("교회를 추가하시겠습니까?")) {
            const response = await axios.post(process.env.REACT_APP_RESTAPI_HOST + "church", params).then(function (res) {
                alert("추가되었습니다.");
                readChurchInfo();
                setaddChurchInfo({
                    userId: window.sessionStorage.getItem("id"),
    ...
    const uploadExcelFile = async () => {
        ...
        if (window.confirm("교회 정보를 업로드하시겠습니까?")) {
            await axios.post(process.env.REACT_APP_RESTAPI_HOST + "church/excel/" + window.sessionStorage.getItem("id"), params).then(function (res) {
        ...
        <div>
            ...
            <div className="admin-content-wrapper">
                <div className="table-wrapper mt0 relative">
                    ...
                    <CommonModal open={isInsertModalOpen} close={insertModalClose} func={createChurch} header="교회 추가하기" footer="추가하기">
                        <div id="church-info" className="lecture-info-wrapper mb40">
                            ...
                            <div>
                                <span className="add-church-title">담임 교역자 성명</span>
                                <input className="p4" type="text" name="pastor" onChange={onAdd} />
                            </div>
                            <div>
                                <span className="add-church-title">실무 교역자 성명</span>
                            ...
                            <div className="mb8" style={{ position: "relative" }}>
                                ...
                                {isPostCodeOpen && (
                                    <Postcode style={{ width: 320, height: 320, position: "absolute", left: "170px" }} jsOptions={{ animated: true }} onSelected={(data) => selectPostCode(data)} />
                                )}
                            </div>
                            <div className="mb8">
                                <span className="add-church-title"></span>
                    ...
                    <div className="mb35">
                        <input
                            ...
                            type="text"
                            placeholder="교회명, 교단, 연관성, 담임 목사명, 실무자명, 전체 주소, 연락처, 이메일, 메모 사항으로 키워드 검색"
                            value={keyword || ""}
                    ...
                    {clickedChurch !== null ? (
                        ...
                        <select className="w158 p4">
                            ...
                            <option>교회 규모</option>
                                ...
                                <div>
                                    <span className="add-church-title">실무교역자 성명</span>
                            ...
                            <span>{clickedChurch.admin_phone}</span>
                                    ...
                                    <div>
                                        <span className="add-church-title">교회 홈페이지</span>
                                        <span>
                                            <a href={clickedChurch.page_url !== "" && clickedChurch.page_url !== null ? clickedChurch.page_url : ""}>{clickedChurch.page_url}</a>
                                        </span>
                                    </div>
                                    <div>
                                        <span className="add-church-title">교회 팩스 번호</span>
                    ...
                    {churchInfo !== null && currentPosts(churchInfo).length > 0 ? (
                        currentPosts(churchInfo).map((church, i) => (
                            <div
                                key={church.id}
                                className={church === clickedChurch ? "table-row-church click-inst-row" : "table-row-church"}
                                onClick={() => {
                                    clickChurch(church);
                                }}
                            >
                                ...
                                <span className="td church-location">
                                    {church.addr1} {church.addr2}
                                </span>
                ...
                <Pagination
                    ...
                    totalItemsCount={churchInfo === null ? 0 : isShowDelete ? churchInfo.filter((data) => data.del_date !== null).length : churchInfo.filter((data) => data.del_date === null).length}
