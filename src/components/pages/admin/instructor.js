...
function AdminInstructor() {
    ...
    function clickInstructor(inst) {
        if (!isEdit) {
            if (clickedInstructor === null || clickedInstructor !== inst) {
                ...
                console.log("==============");
                // clickedInstructor.intro = clickedInstructor.intro.replace(/<br\/>/g, "\r\n");
                // clickedInstructor.memo = clickedInstructor.memo.replace(/<br\/>/g, "\r\n");
                // console.log(clickedInstructor.intro);
                // console.log(clickedInstructor.memo);
                // console.log(clickedInstructor);
                // console.log(clickedInstructor[intro]);
                // console.log(inst["memo"]);
                // inst["memo"].value = inst["memo"].value.replace(/<br\/>/g, "\r\n");
    ...
    const updateInstructor = async () => {
        if (window.confirm("프로필 정보를 수정하시겠습니까?")) {
            ...
            editInfo.intro = editInfo.intro.replace(/<br\/>/g, "\r\n");
            editInfo.memo = editInfo.memo.replace(/<br\/>/g, "\r\n");
    ...
    const createInstructor = async () => {
        if (window.confirm("강사를 추가하시겠습니까?")) {
            ...
            addInfo.intro = addInfo.intro.replace(/<br\/>/g, "\r\n");
            addInfo.memo = addInfo.memo.replace(/<br\/>/g, "\r\n");
                    ...
                    {clickedInstructor ? (
                        isEdit ? (
                            <div className="show-inst-detail relative">
                                ...
                                <div className="profile-edit">
                                    <div className="profile-image">
                                        {removeProfileImage === false && profileImage !== null ? (
                                          ...
                                          clickedInstructor.image !== "null" &&
                                          clickedInstructor.image !== null &&
                                          clickedInstructor.image !== "" ? (
                                            <img
                                                className="profile-img-content"
                                ...
                                <textarea name="intro" className="h100 mb20 inst-textarea" value={editInfo.intro.replace(/<br\/>/g, "\r\n")} onChange={onEdit}></textarea>
                                <p className="mb10">약력 작성란</p>
                                <textarea name="memo" className="h180 mb20 inst-textarea" value={editInfo.memo.replace(/<br\/>/g, "\r\n")} onChange={onEdit}></textarea>
                            ...
                            <div className="show-inst-detail relative">
                                ...
                                <div className="mb25">
                                    ...
                                    {clickedInstructor}
                                    {/* settingInfo .find((element) => element.key === "section2_text") .value.split("
                                    <br />
                                    ") .map((item, i) => <div key={i}>{item}</div>) value={editInfo.memo.replace(/<br\/>/g, "\r\n")} */}
                                    hello
                                </div>
                                {isShowDelete ? (
